...
public extension Keychain.Certificates {
    ...
    /// Deletes certificates matching the specified criteria.
    ///
    /// Removes all certificates from the Keychain that match the provided search parameters.
    ///
    /// - Parameters:
    ///   - issuer: Filter by certificate issuer (normalized issuer sequence).
    ///   - serialNumber: Filter by serial number.
    ///   - label: Filter by label.
    ///   - accessGroup: The access group scope of the certificates to delete. Use `.specific(...)` to target a specific
    ...
    ///
    /// - Returns: `true` if at least one certificate was deleted, `false` if no matching certificates were found.
    ///
