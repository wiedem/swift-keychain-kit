...
public extension Keychain.Certificates {
    /// Gets the attributes of a certificate using an item reference.
    ///
    /// Retrieves the metadata for the certificate identified by the given ``ItemReference``. The reference uniquely
    /// identifies a specific Keychain item, so this method guarantees at most one result.
    ///
    /// - Parameters:
