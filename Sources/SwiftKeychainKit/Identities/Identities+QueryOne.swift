...
public extension Keychain.Identities {
    /// Queries for a single identity matching the specified label.
    ///
    /// This is a convenience method that returns a single matching identity or `nil` if none found. If multiple identities
    /// match the criteria, a ``KeychainError/multipleItemsFound`` error is thrown to indicate that the query parameters are not
    /// specific enough.
    ///
    /// - Parameters:
    ///   - label: The identity label to match.
    ...
    ///   - synchronizable: Whether to query for a synchronized identity. Defaults to `false`.
    ...
    ///
    /// - Returns: The matching [SecIdentity](https://developer.apple.com/documentation/security/secidentity), or `nil`
    ///   if not found (or skipped due to `skipIfUIRequired`).
    ///
    /// - Throws:
    ///   * ``KeychainError/multipleItemsFound`` if more than one identity matches the criteria.
    ///   * ``KeychainError`` for other Keychain operation failures.
    ///
    /// - Note: Security Consideration: The returned identity may represent sensitive material.
