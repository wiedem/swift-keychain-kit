...
public extension Keychain.Identities {
    /// Gets an identity using an item reference.
    ///
    /// Retrieves the identity identified by the given ``ItemReference``. The reference uniquely identifies
    ...
    static func get(
        ...
        let results: [SecIdentity] = try Keychain.queryItems(query: query, limit: .one)
